						...
						<table class="table table-bordered table-striped text-center" id="transactionsTable">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">@if($allTransaction->transaction_type == APP\Transaction::BUY || $allTransaction->transaction_type == APP\Transaction::REPURCHASE || $allTransaction->transaction_type == APP\Transaction::CANCELLED){{ $allTransaction->transaction_type }} {{ $allTransaction->transaction_description }} @else @if($allTransaction->transaction_type == APP\Transaction::DIVIDEND) {{ round($allTransaction->rate ,2) }} CENTS PER SHARE @elseif($allTransaction->transaction_type == APP\Transaction::FIXED_DIVIDEND)  {{ round($allTransaction->rate ,2) }} % FIXED DIVIDEND @elseif($allTransaction->transaction_type == APP\Transaction::ANNUALIZED_DIVIDEND) {{ $allTransaction->transaction_description }} @endif @endif</td>
