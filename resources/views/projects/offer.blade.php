									...
									<div class="row" id="section-1">
										<div class="col-md-12">
											<div>
												...
												<input required type="number" name="apply_for" class="form-control" placeholder="" @if(isset($eoi)) value="{{number_format(round($eoi->investment_amount * $project->share_per_unit_price, 2))}}" @endif @if(isset($clientApplication)) value="{{$clientApplication->investment_amount }}" @endif style="width: 60%;" id="application_money" step="0.01" min="{{$project->investment->minimum_accepted_amount * $project->share_per_unit_price}}"><br>
												<label>I/We apply for *</label>
												<input type="text" readonly name="amount_to_invest" class="form-control" placeholder="Minimum Amount {{$project->investment->minimum_accepted_amount}}" style="width: 60%" id="apply_for" min="{{$project->investment->minimum_accepted_amount}}"  required @if(isset($eoi)) value="{{$eoi->investment_amount}}" @endif @if(isset($clientApplication)) value="{{$clientApplication->investment_amount / $project->share_per_unit_price}}" @endif>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		amount.bind('keyup mouseup', function () {
			let value = (amount.val() / parseFloat({{ $project->share_per_unit_price}})).toFixed(4);
			$("#apply_for").val(value);
		});
	});
	$(document).ready( function() {
