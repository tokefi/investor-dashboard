            ...
            <form action="{{ route('custom-field.store') }}" method="POST">
                ...
                <div class="row form-group">
                    <div class="col-sm-4" style="margin-bottom: 20px;">
                        ...
                        <div class="col-sm-4" style="margin-bottom: 20px;">
                            <label for="label">Label<sup class="text-danger">*</sup>
                            </label>
                            <input type="text" class="form-control" id="label" name="label" placeholder="Enter label" required>
                        </div>
                        <div class="col-sm-4" style="margin-bottom: 20px;">
                            <label for="section">
                                Select section<sup class="text-danger">*<sup>
                                </label>
                            ...
                            <div class="col-sm-4 hide nameField" style="margin-bottom: 20px;">
                                <label for="radioField">
                                Select Radio button<sup class="text-danger">*<sup>
                                </label>
                                <select class="form-control" id="radioField" name="radioField" >
                                    <option value="">-- Select/Add new button --</option>
                                    @foreach ($customFields->where('type','radio') as $section)
                                    <option value="{{ $section->name }}">{{ $section->label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-sm-4 buttonValue hide" style="margin-bottom: 20px;">
                                <label for="label">Button Value<sup class="text-danger">
                                </label>
                                <input type="text" class="form-control" id="buttonValue" name="buttonValue" placeholder="Enter value" >
                            </div>
                            <div class="col-sm-4 hide masterField" style="margin-bottom: 20px;">
                                <label for="label">Master Field<sup class="text-danger">
                                </label>
                                <input type="text" class="form-control" id="masterField" name="masterField" placeholder="Enter label" >
                            </div>
                            <div class="col-sm-4 radioMasterField" style="margin-bottom: 20px;">
                                <label for="radioMasterField">Radio Master Field<sup class="text-danger">
                                </label>
                                <select class="form-control" id="radioMasterField" name="radioMasterField" >
                                    <option value="">-- Select --</option>
                                    @foreach ($customFields->where('type','radio') as $section)
                                    @foreach($section->radioField as $field)
                ...
                <table class="table table-borderless table-striped table-responsive">
                    ...
                    <tbody>
                        ...
                        <td><input type="text" name="customFieldLabels[]" value="{{ $customField->label }}" style="border:0px;  background-color: transparent;">
                            @if($customField->radioField->count())
                                <span class="">Values</span>
                                <hr style="border: 1px solid #333;margin-bottom: -14px;margin-top: 5px; width: 240px; margin-left:0px; ">
                                @foreach($customField->radioField as $field)
                                <br>
                                <input type="text" name="radioFieldLabels[]" value="{{ $field->label }}" style="border:0px;  background-color: transparent;"><input type="text" name="radioFieldvalues[]" value="{{ $field->value }}" style="border:0px;  background-color: transparent;">
                                @endforeach
                                <input type="hidden" name="customIds" value="{{ $customField->id }}">
                            @endif
                         </td>
